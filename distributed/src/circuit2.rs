...
use jf_primitives::merkle_tree::{FilledMTBuilder, MerkleLeafProof, MerklePath, NodePos};
...
impl Gate {
    ...
    pub fn q_c(&self) -> Fr {
        match self.gate_type {
            ...
            GateType::RescueAffine(r, i) | GateType::Power5NonLinear(r, i) => ROUND_KEYS[r as usize][i as usize],
            _ => Default::default(),
        }
    }
...
impl PlonkCircuit {
    ...
    fn add_constant_state(
        ...
        let vars: Result<Vec<Variable>, PlonkError> = input_var
            ...
            .map(|(i, &var)| {
                ...
                self.insert_gate(&[var, self.one(), 0, 0, output_var], GateType::RescueAddConstant(i as u8))?;
    ...
    fn affine_transform(
        ...
        let output_val = MDS.zip(ROUND_KEYS[r]).map(|(row, k)| {
            row.iter().zip(input_var).fold(k, |acc, (&a, &b)| acc + a * self.witness(b).unwrap())
        });
