...
use ark_ff::{BigInteger, Field, One, PrimeField, UniformRand, Zero, FftField};
use ark_poly::{
...
use jf_plonk::{
    circuit::{Arithmetization, gates::Gate, Variable, GateId, WireId},
...
impl Prover {
        ...
        R: CryptoRng + RngCore,
    {
        ...
        let quotient_poly = {
            ...
            let g3 =
                &wire_polys[4] + &DensePolynomial { coeffs: vec![gamma, beta * prove_key.vk.k[4]] };
...
mod tests {
    ...
    use jf_plonk::{
        prelude::{Circuit, PlonkCircuit},
        proof_system::{PlonkKzgSnark, Snark}, circuit::gates::PaddingGate,
    };
    use jf_primitives::{merkle_tree::{FilledMTBuilder, MerkleLeafProof}, circuit::merkle_tree::{AccElemVars, MerkleTreeGadget}};
    use rand::Rng;
    ...
    pub fn generate_circuit<R: Rng>(rng: &mut R) -> Result<PlonkCircuit<Fr>, PlonkError> {
        ...
        let parts = (0..NUM_MEMBERSHIP_PROOFS)
            ...
            .map(|uid| {
                ...
                let (_, MerkleLeafProof { leaf, path }) =
                    mt.get_leaf(uid as u64).expect_ok().unwrap();
                ...
                let claimed_root_var =
                    circuit.compute_merkle_root(acc_elem_var, &path_var).unwrap();
