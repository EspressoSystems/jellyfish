...
pub trait RescueGadget<F: RescueParameter> {
    ...
    ) -> Result<RescueStateVar, PlonkError>;
...
pub(crate) trait RescueHelperGadget<F: PrimeField>: Circuit<F> {
    fn check_var_bound_rescue_state(&self, rescue_state: &RescueStateVar)
        -> Result<(), PlonkError>;
    ...
    F: RescueParameter,
{
    ...
    fn affine_transform(
        ...
        let input_val_fields_elems: Result<Vec<F>, PlonkError> =
...
mod tests {
    ...
    use crate::circuit::{Circuit, PlonkCircuit, Variable};
